...
function inicializar_simbolos() {
	...
	if (no_terminales.indexOf(simbolo_inicial) < 0) {
		alert('El símbolo inicial no esta en  No-Terminales');
		return;
	}
...
function insertar_produccion_tabla(X, Y) {
	...
	var tbl_produccionespunt = document.getElementById('tbl_produccionespunt');
	...
	tr1.innerHTML = '<td>' + X + ' -> ' + '.'+Y.split('').join(' ') + '</td>';

	tbl_produccionespunt.appendChild(tr1);
}
...
function insertar_produccion() {
	...
	if (error) {
		alert('El símbolo ' + error + ' no existe en los símbolos terminales ni en los no-terminales');
		return;
	}
